...
const ProductsDetails = ({ goalId }) => {
   ...
   const updater = (e) => {
      ...
      const url = `https://myshop-server.iran.liara.run/api/update-product/${goalId}`;
      axios
         .post(url, formData)
         .then((d) => {
   ...
   const remover = () => {
      const url = `https://myshop-server.iran.liara.run/api/delete-product/${goalId}`;
      axios
         .post(url)
         .then((d) => {
   ...
   useEffect(() => {
      axios
         .get(
            `https://myshop-server.iran.liara.run/api/get-product-by-id/${goalId}`
         )
         .then((d) => {
            setfullData(d.data);
            setTag(d.data.tags);
               ...
               >
                  ...
                  <div className=" flex flex-col gap-2">
                     ...
                     <select
                        ...
                        className=" p-2 rounded-md w-full outline-none border-2 border-zinc-300 focus:border-orange-400"
                     >
                        {fullData.typeOfProductRef && fullData.typeOfProductRef == "book" ? (
                           <>
