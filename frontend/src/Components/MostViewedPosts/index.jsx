...
const MostViewedPosts = () => {
   ...
   useEffect(() => {
      axios
         .get("https://myshop-server.iran.liara.run/api/get-most-viewed-posts")
         .then((d) => {
            setposts(d.data);
         })
         ...
         <ul className=" flex flex-col gap-3">
            {posts[0] == -1 ? (
               ...
               posts.map((po, i) => (
                  <li key={i}>
                     <Link
                        className=" p-2 flex justify-start items-center text-base  sm:text-sm border-r-2 border-zinc-600 hover:border-indigo-600"
                        href={`/blog/${po.slug}`}
                     >
                        {po.title}
                     </Link>
